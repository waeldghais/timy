...
initState(){
  ...
  Future.delayed(Duration(seconds: 0),()async{
    await getStatus();
  final response= await flutterLocalNotificationsPlugin.getNotificationAppLaunchDetails();
...
showNotification(String title,DateTime showTime) async {
    var android = AndroidNotificationDetails(
        'channel id', 'channel name', 'channel description',enableVibration: true,playSound: true,importance:Importance.High,priority: Priority.High );
    var iOS = IOSNotificationDetails();
    var platform = NotificationDetails(android, iOS);
        new DateTime.now().add(Duration(seconds: 10));
    await flutterLocalNotificationsPlugin.schedule(
      int.parse(widget.id), title, 'The show is about to start in half an hour',showTime.subtract(Duration(minutes: 30)), platform,
      payload: widget.id);
  }
...
isScheduled=true;
  ...
  createAlertDialoge(BuildContext context,DateTime premier,DateTime replay,String title) {
    return showDialog(context: context,barrierDismissible: false,builder: (context){
      return AlertDialog(
        ...
        content: StatefulBuilder(
            builder:(context,setState)=> Container(height: 120, 
            child: Column
            (children: [
              ...
              title: Text('Premier Show', style: TextStyle(color: Colors.white),),
              ...
              title: Text('Replay Show', style: TextStyle(color: Colors.white),),
    ...
    return Scaffold(
      appBar: AppBar(   
        title: Row(
          ...
          children: <Widget>[
            Image.asset('assets/appbar_logo45.png', fit: BoxFit.cover, height: 42,),
          ],
        ),
...
/////////////// Schedule info container  ////////////////////////////////////////////////////
              Padding(
                padding: EdgeInsets.only(top: 300.0, bottom: 1),
                child: ListView(
                  children: <Widget>[
                    Padding(
              ...
              Positioned(
                ...
                child: Container(
                  ...
                  child: IconButton(
                    ...
                    onPressed:!isScheduled? () async{
                      await createAlertDialoge(context,shows.premier,shows.replay1,shows.showTitle);
                      // setState(() {
                      ...
                      //   // }
                      // });
                      if(selectedValue==0)Navigator.of(context).pop();
                      else{
                        setState(() {
                          isScheduled=true;
                        });
                        Navigator.of(context).push(MaterialPageRoute(builder: (_)=>Schedule()));
                      }
                    }:()async{
                      DBHelper.deleteData('schedule', widget.id);
                     await flutterLocalNotificationsPlugin.cancel(int.parse(widget.id));
                      setState(() {
                        isScheduled=false;
                      });
                    },
