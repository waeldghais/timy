...
class DBHelper{
  static Future<Database> database() async{
    ...
    return sql.openDatabase(path.join(dbPath,'schedule.db'),onCreate: (db,version){
      return db.execute(
'CREATE TABLE schedule(id TEXT PRIMARY KEY,date TEXT,compare TEXT)'
  ...
  static Future<List<Map<String,Object>>> getDataByID(String table,String id) async{
    final db=await DBHelper.database();
    return db.query(table,where: 'id=?',whereArgs: [id]);
  }

   static Future<List<Map<String,Object>>> getDataByDate(String table,String date) async{
    final db=await DBHelper.database();
  ...
  static Future<void> deleteOldData(String table) async{
    ...
    allData.forEach((data)async{ 
      if(DateTime.now().subtract(Duration(hours: 7)).isAfter(DateTime.parse(data['date'])))
