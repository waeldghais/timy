...
// here i am fetching data from firebase
  ...
  Future<void> getCurrentAndNextPlayingShows() async {
    ...
    scheduledData.forEach((data) {
      ...
      if (_currentlyPlaying.isEmpty) {
        if (DateTime.parse(data['date']).isBefore(DateTime.now()) &&
            DateTime.parse(data['date'])
                .isBefore(DateTime.now().add(Duration(hours: 1))) &&
            DateTime.now().difference(DateTime.parse(data['date'])) <=
                Duration(hours: 2)) {
    ...
    scheduledData.forEach((data) {
      ...
      if (_nextPlaying.isEmpty) {
        if (DateTime.parse(data['date']).isAfter(DateTime.now()) &&
            DateTime.parse(data['date'])
                .isBefore(DateTime.now().add(Duration(hours: 23)))) {
  ...
  List<Shows> getChannelShows(String channelId) {
    return _shows
        .where((prodItem) => prodItem.channelUid == channelId)
        .toList();
  }
